...
- [`elk-layout.ts`](elk-layout.ts.md) — recursive ELK placement and Svelte Flow conversion, wrapped in a bounded, deterministic layout/routing feedback loop that escalates spacing when [`routing.ts`](routing.ts.md)'s routing-quality cost shows genuine structural defects.
