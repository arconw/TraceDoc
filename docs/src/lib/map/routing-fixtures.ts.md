...
Deterministic `ProjectModel`-shaped stress graphs for the architecture map router/layout, and the single source of truth both `map-graph.test.mjs`'s stress-fixture assertions and `scripts/generate-map-fixtures.mjs`'s on-disk debug workspaces build from. Each fixture models a named real-world map scenario (fan-in, fan-out, corridors, cross-folder highway, nesting, crossing-heavy, a mixed hub, noise near a corridor, and an incremental-change pair) instead of a generic synthetic benchmark.
