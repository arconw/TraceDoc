...
| `dense-skip-chain`        | 18 documents, one folder, sequential + 4-ahead skip links | Deliberately poor initial layout (real crossings from tightly packed skip-ahead links); the phase-5 layout/routing feedback pass must reduce `routing.ts`'s `computeRoutingCost` relative to a single unadjusted ELK pass. |
