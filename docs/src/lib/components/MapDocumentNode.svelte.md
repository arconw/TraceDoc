...
- `portClass(port)` / `portStyle(port)` — per-port CSS state and placement. A port whose `linkId` matches `data.activeEdgeId` (the currently hover/keyboard-traced edge, set by [`MapView`](../views/MapView.svelte.md)) gets `map-port-active` — larger, full opacity, the active edge color, in both normal and `forced-colors` themes. Otherwise, while this node is `emphasis: 'active'` (hovered, selected, or an endpoint of the traced edge) OR `emphasis: 'connected'` (a neighbor of the hovered/selected document, reachable through one of its links — see `nodeEmphasis` in `map-view-state.ts`), its ports get `map-port-emphasized` for a lighter visibility bump; `nodeEmphasized` treats both states identically so a connected neighbor's ports become visible together with the hovered document's own, instead of only the hovered document lighting up. Idle ports stay small and partially visible rather than fully hidden, since a rendered port always represents a real connection; a local `:hover`/`:focus-within` CSS rule on the node also raises all its own ports' opacity immediately, without waiting on the `data.emphasis` round-trip through `MapView`.
