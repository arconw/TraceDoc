...
- `activate(root,project)` — cleans owned crash artifacts, increments generation, installs new active state at revision zero.
...
- `windows_recovery_matches(artifact)` — verifies held directory/path identity, named slot bytes, and held artifact identity. Current implementation checks named slot existence/content but obtains identity from the held artifact handle.
