...
- `persist_windows_recovery(...)` — creates/opens owned recovery directory, serializes transaction lock, classifies/refreshes/installs deterministic artifact. The directory handle is opened with `FILE_SHARE_READ | FILE_SHARE_WRITE` and kept open for the whole transaction; `FILE_SHARE_WRITE` is required even though this process only ever renames files into that directory from other handles it owns itself - NTFS enforces directory share flags against a process's own concurrent handles, not just external ones, so `FILE_SHARE_READ` alone made every `windows_move_file` call into the directory (install → slot) fail with `ERROR_SHARING_VIOLATION` regardless of external contention. Confirmed by reproducing the failure and the fix directly against a live NTFS volume, isolated from the app. On the freshly-installed-artifact path, `windows_recovery_matches` is called (via `fs::read` on a new handle) before `lock_windows_handle`, not after: an exclusive `LockFileEx` byte-range lock blocks reads through any other handle to the same file, including one opened later by this same process, so locking first made the immediately-following self-verification read fail every time with `ERROR_LOCK_VIOLATION` and surface as "Unable to verify persisted recovery". The classify/refresh branch above already verified before locking; this path now matches it.
...
- `windows_recovery_matches(artifact)` — re-derives directory and recovery-slot identity from their current on-disk paths (not the held handles) and compares them against the captured baseline alongside slot bytes, so a slot replaced out from under the held handle is detected instead of trivially matching itself. Reads the slot bytes back through the artifact's own already-open, already-locked handle (`read_file_bytes_portable`, seek-to-start + read), not a fresh `fs::read` by path: once the artifact is locked, an exclusive `LockFileEx` byte-range lock blocks reads through any *other* handle to that file, including one this same process opens later, so a fresh-handle read would fail every time (`ERROR_LOCK_VIOLATION`) regardless of whether the content actually still matches. Takes `&mut` for this reason. Identity checks stay path/handle-based (`GetFileInformationByHandle` queries are metadata-only and aren't blocked by the lock).
