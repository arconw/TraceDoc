...
- `WorkspaceScanner::scan_directory(path,relative,parentId)` — sorted directory traversal; creates hierarchy records and Markdown documents.
...
- `ignored_relative_ancestor(root,relative)` — returns first ignored or unsafe ancestor for incremental events.
